<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<%@ taglib prefix="tiles" uri="http://tiles.apache.org/tags-tiles" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib prefix="sec" uri="http://www.springframework.org/security/tags"%>

<div class="mypage-aside-con">
	<div class="mem-ul-set">
		<ul class="mem-ul">
			<span class="span-pd">
				<svg width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
					<path d="M23.7 20.308a11.919 11.919 0 0 0-5.342-7.466 8 8 0 1 0-12.709 0 11.894 11.894 0 0 0-5.344 7.465 3.01 3.01 0 0 0 .582 2.561A2.977 2.977 0 0 0 3.23 24h17.54a2.977 2.977 0 0 0 2.343-1.132 3.008 3.008 0 0 0 .587-2.56ZM12 2a6 6 0 1 1-6 6 6.006 6.006 0 0 1 6-6Zm9.549 19.623a.982.982 0 0 1-.779.377H3.23a.982.982 0 0 1-.779-.377 1.026 1.026 0 0 1-.2-.87A9.9 9.9 0 0 1 7.1 14.306a7.949 7.949 0 0 0 9.813 0 9.925 9.925 0 0 1 4.838 6.45 1.024 1.024 0 0 1-.202.867Z" data-name="31. User" fill="#444444" class="fill-000000"></path>
				</svg>
			</span>
			<a href="/member/mypage" class="sub-title">프로필</a>
		</ul>
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 846.66 846.66" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd">
					<path d="M681.83 484.09c0-27.16 41.3-27.16 41.3 0v331.26c0 11.4-9.25 20.64-20.65 20.64H50.84c-11.4 0-20.65-9.24-20.65-20.64V31.31c0-11.4 9.25-20.64 20.65-20.64h507.83c90.49 0 164.46 73.96 164.46 164.45v91.1c0 27.15-41.3 27.15-41.3 0v-91.1c0-67.7-55.46-123.16-123.16-123.16H71.48v631.33h610.35v-199.2zM452.01 633.74c-27.16 0-27.16-41.3 0-41.3h158.98c27.15 0 27.15 41.3 0 41.3H452.01zm0-76.4c-27.16 0-27.16-41.29 0-41.29h158.98c27.15 0 27.15 41.29 0 41.29H452.01zM71.48 724.58v70.12h610.35v-70.12H71.48zm189.56-378.86c64.2 0 116.65 52.45 116.65 116.66v98.06c0 11.4-9.25 20.65-20.65 20.65H165.03c-11.4 0-20.64-9.25-20.64-20.65v-98.06c0-64.21 52.44-116.66 116.65-116.66zm0 41.3c-41.41 0-75.36 33.95-75.36 75.36v77.42H336.4v-77.42c0-41.41-33.96-75.36-75.36-75.36zm0-256.02c52.45 0 94.97 42.53 94.97 94.98s-42.52 94.97-94.97 94.97c-52.45 0-94.97-42.52-94.97-94.97 0-52.45 42.52-94.98 94.97-94.98zm0 41.29c-29.65 0-53.68 24.04-53.68 53.69 0 29.64 24.03 53.68 53.68 53.68s53.68-24.04 53.68-53.68c0-29.65-24.03-53.69-53.68-53.69zm312.92 188.89c-17.06-20.98 14.88-46.95 31.94-25.97l42.21 51.86 134.71-108.06c21.08-16.95 46.88 15.15 25.8 32.1L657.96 431.97c-8.82 7.08-21.72 5.72-28.87-3.06l-55.13-67.73z" fill="#444444" fill-rule="nonzero" class="fill-000000"></path>
				</svg>
			</span>
			<a href="#" class="sub-title ">이력서</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/mypageMem/memberCreateResume" class="sub-inner my-title">이력서 등록</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/mypageMem/memberListResume" class="sub-inner my-title">이력서 목록</a>
			</li>
		</ul>	
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 846.66 846.66" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd">
					<path d="M117.25 10.39h612.16c11.4 0 20.64 9.24 20.64 20.64v784.6c0 11.4-9.24 20.64-20.64 20.64H117.25c-11.4 0-20.64-9.24-20.64-20.64V31.03c0-11.4 9.24-20.64 20.64-20.64zm379.9 637.26c-27.15 0-27.15-41.29 0-41.29h117.48c27.15 0 27.15 41.29 0 41.29H497.15zm0 88.37c-27.15 0-27.15-41.29 0-41.29h117.48c27.15 0 27.15 41.29 0 41.29H497.15zM232.03 584.8h144.42c11.4 0 20.65 9.25 20.65 20.65v131.49c0 11.4-9.25 20.64-20.65 20.64H232.03c-11.4 0-20.64-9.24-20.64-20.64V605.45c0-11.4 9.24-20.65 20.64-20.65zm123.77 41.29H252.68v90.2H355.8v-90.2zm-156.66-92.68c-27.16 0-27.16-41.29 0-41.29h446.19c27.15 0 27.15 41.29 0 41.29H199.14zm0-86.22c-27.16 0-27.16-41.29 0-41.29h446.19c27.15 0 27.15 41.29 0 41.29H199.14zm315.14-278.58c-27.16 0-27.16-41.29 0-41.29h109.93c27.15 0 27.15 41.29 0 41.29H514.28zm0 177.82c-27.16 0-27.16-41.29 0-41.29h109.93c27.15 0 27.15 41.29 0 41.29H514.28zm0-88.91c-27.16 0-27.16-41.29 0-41.29h109.93c27.15 0 27.15 41.29 0 41.29H514.28zm-183.2-145.83c54.7 0 99.05 44.35 99.05 99.05 0 13.86-2.85 27.06-7.99 39.04 24.37 24.23 38.21 57.22 38.21 91.64 0 27.15-41.29 27.15-41.29 0 0-21.3-7.37-41.39-21.25-57.48-17.61 16.06-41.03 25.85-66.73 25.85-25.7 0-49.12-9.79-66.72-25.85-13.89 16.09-21.26 36.19-21.26 57.48 0 27.15-41.29 27.15-41.29 0 0-34.42 13.84-67.41 38.21-91.64-5.14-11.98-7.99-25.18-7.99-39.04 0-54.7 44.35-99.05 99.05-99.05zm0 41.29c-31.9 0-57.76 25.86-57.76 57.76s25.86 57.76 57.76 57.76 57.76-25.86 57.76-57.76-25.86-57.76-57.76-57.76zm377.68-101.3H137.9v743.3h570.86V51.68z" fill="#444444" fill-rule="nonzero" class="fill-000000"></path>
				</svg>
			</span>
			<a href="#" class="sub-title">자기소개서</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/mypageMem/portfolioCreate" class="sub-inner my-title">자소서 기본양식 등록</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/mypageMem/portfolioFreeCreate" class="sub-inner my-title">자소서 자유양식 등록</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/mypageMem/portfolioList" class="sub-inner my-title">자기소개서 목록</a>
			</li>
		</ul>
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 846.66 846.66" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd">
					<path d="M293.76 130.09H552.9V30.08c0-11.43 9.27-20.71 20.71-20.71h119.84c11.43 0 20.71 9.28 20.71 20.71v100.01h51.57c11.44 0 20.71 9.27 20.71 20.71v665.78c0 11.43-9.27 20.71-20.71 20.71H80.93c-11.44 0-20.71-9.28-20.71-20.71V150.8c0-11.44 9.27-20.71 20.71-20.71h51.58V30.08c0-11.43 9.27-20.71 20.7-20.71h119.85c11.43 0 20.7 9.28 20.7 20.71v100.01zM101.64 334.21h643.39v-33.99H101.64v33.99zm643.39 41.41H101.64v420.25h643.39V375.62zM101.64 258.8h643.39v-87.29h-30.87v34.45c0 11.44-9.28 20.71-20.71 20.71H573.61c-11.44 0-20.71-9.27-20.71-20.71v-34.45H293.76v34.45c0 11.44-9.27 20.71-20.7 20.71H153.21c-11.43 0-20.7-9.27-20.7-20.71v-34.45h-30.87v87.29zm59.18 369.67h74.19c11.43 0 20.71 9.27 20.71 20.71v74.19c0 11.43-9.28 20.7-20.71 20.7h-74.19c-11.43 0-20.71-9.27-20.71-20.7v-74.19c0-11.44 9.28-20.71 20.71-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zm397.35-41.42h74.19c11.43 0 20.71 9.27 20.71 20.71v74.19c0 11.43-9.28 20.7-20.71 20.7h-74.19c-11.43 0-20.71-9.27-20.71-20.7v-74.19c0-11.44 9.28-20.71 20.71-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zm-203.76-41.42h74.19c11.44 0 20.71 9.27 20.71 20.71v74.19c0 11.43-9.27 20.7-20.71 20.7h-74.19c-11.43 0-20.7-9.27-20.7-20.7v-74.19c0-11.44 9.27-20.71 20.7-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zM311.1 628.47h74.18c11.44 0 20.71 9.27 20.71 20.71v74.19c0 11.43-9.27 20.7-20.71 20.7H311.1c-11.44 0-20.71-9.27-20.71-20.7v-74.19c0-11.44 9.27-20.71 20.71-20.71zm53.47 41.42h-32.76v32.77h32.76v-32.77zm-203.75-195.5h74.19c11.43 0 20.71 9.27 20.71 20.71v74.18c0 11.44-9.28 20.71-20.71 20.71h-74.19c-11.43 0-20.71-9.27-20.71-20.71V495.1c0-11.44 9.28-20.71 20.71-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zm397.35-41.42h74.19c11.43 0 20.71 9.27 20.71 20.71v74.18c0 11.44-9.28 20.71-20.71 20.71h-74.19c-11.43 0-20.71-9.27-20.71-20.71V495.1c0-11.44 9.28-20.71 20.71-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zm-203.76-41.42h74.19c11.44 0 20.71 9.27 20.71 20.71v74.18c0 11.44-9.27 20.71-20.71 20.71h-74.19c-11.43 0-20.7-9.27-20.7-20.71V495.1c0-11.44 9.27-20.71 20.7-20.71zm53.48 41.42h-32.77v32.77h32.77v-32.77zM311.1 474.39h74.18c11.44 0 20.71 9.27 20.71 20.71v74.18c0 11.44-9.27 20.71-20.71 20.71H311.1c-11.44 0-20.71-9.27-20.71-20.71V495.1c0-11.44 9.27-20.71 20.71-20.71zm53.47 41.42h-32.76v32.77h32.76v-32.77zM252.35 50.79h-78.43v134.46h78.43V50.79zm420.39 0h-78.42v134.46h78.42V50.79z" fill="#444444" fill-rule="nonzero" class="fill-000000"></path>
				</svg>
			</span>
			<a href="/member/calendar" class="sub-title">나의 일정</a>
<!-- 			<li> -->
<!-- 				<i class="fa-solid fa-check"></i> -->
<!-- 				<a href="/member/calendar" class="sub-inner my-title">캘린더</a> -->
<!-- 			</li> -->
		</ul>	
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
					<path d="M25 23h-.021a1.002 1.002 0 0 1-.94-.726L20.87 11.19l-1.935 5.16A1 1 0 0 1 18 17h-4v-2h3.307l2.757-7.351a1 1 0 0 1 1.898.076l3.111 10.892 1.979-5.933A.999.999 0 0 1 28 12h4v2h-3.28l-2.772 8.316A.998.998 0 0 1 25 23ZM15 30h-2v-7a3.003 3.003 0 0 0-3-3H6a3.003 3.003 0 0 0-3 3v7H1v-7a5.006 5.006 0 0 1 5-5h4a5.006 5.006 0 0 1 5 5ZM8 8a3 3 0 1 1-3 3 3 3 0 0 1 3-3m0-2a5 5 0 1 0 5 5 5 5 0 0 0-5-5Z" fill="#444444" class="fill-000000"></path><path data-name="<Transparent Rectangle>" d="M0 0h32v32H0z" fill="none"></path>
				</svg>
			</span>
			<a href="#" class="sub-title">활동내역</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myBoardInquiry" class="sub-inner my-title">문의사항</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myBoardQNA" class="sub-inner my-title">취업Q&A</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myEntReview" class="sub-inner my-title">기업리뷰</a>
			</li>
		</ul>	
		<ul class="mem-ul">
			<span>
				<i class="fa-solid fa-user-pen" style="color: #444444;"></i>
			</span>
			<a href="#" class="sub-title">상담기록</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/consultantList" class="sub-inner my-title">상담사</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/consultantLog" class="sub-inner my-title">나의상담기록</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/consultantReview" class="sub-inner my-title">리뷰목록</a>
			</li>
		</ul>	
		
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
					<path d="M415.054 119.15a102.244 102.244 0 0 0-144.595 0L256 133.608l-14.46-14.46A102.244 102.244 0 0 0 96.947 263.744L256 422.797l159.054-159.053a102.244 102.244 0 0 0 0-144.595Z" fill="#444444" class="fill-000000"></path>
				</svg>
			</span>
			<a href="#" class="sub-title">관심정보</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myEnterpriseList" class="sub-inner my-title">관심기업</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myRecruit" class="sub-inner my-title">스크랩 채용공고</a>
			</li>
		</ul>	
		<ul class="mem-ul">
			<span>
				<svg width="20" height="20" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
					<path d="m35.364 32.373 10.133-6.432a14.084 14.084 0 0 0 6.337-11.927c0-5.918-4.164-11.065-9.663-11.457a10.12 10.12 0 0 0-9.197 4.721.697.697 0 0 1-1.194 0 10.083 10.083 0 0 0-9.945-4.64c-5.181.757-8.915 5.76-8.915 11.376a14.084 14.084 0 0 0 6.337 11.927l10.134 6.432a5.524 5.524 0 0 0 5.973 0ZM15.105 37.15H8.547a1.043 1.043 0 0 0-1.043 1.043v22.382a1.044 1.044 0 0 0 1.043 1.044h6.558a2.274 2.274 0 0 0 2.275-2.275v-19.92a2.274 2.274 0 0 0-2.275-2.274ZM56.328 39.778a3.804 3.804 0 0 0-3.185-1.716h-2.57a5.55 5.55 0 0 0-5.145 3.442 5.761 5.761 0 0 1-1.024 1.62 8.378 8.378 0 0 1-6.684 3.8h-7.612a1.35 1.35 0 0 1 0-2.7h7.576a5.104 5.104 0 0 0 2.725-.87v-.053a5.22 5.22 0 0 0-5.219-5.239h-5.586a7.738 7.738 0 0 0-5.496 2.27l-.144.143a7.248 7.248 0 0 1-3.885 2.013V57.4H40.45a15.43 15.43 0 0 0 14.19-9.347l2.007-4.67a3.803 3.803 0 0 0-.32-3.604Z" fill="#444444" class="fill-000000"></path>
				</svg>
			</span>
			<a href="#" class="sub-title ">지원관리</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myApplicationList" class="sub-inner my-title">입사지원 </a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/member/myJobOffer" class="sub-inner my-title">제안 받은 기업</a>
			</li>
		</ul>	
		<ul class="mem-ul">
			<span>
			<svg width="20" height="20" viewBox="0 0 48 48" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">
				<g fill="#444444" class="fill-241f20"><path d="M22.259 35.893v-2.721c3.07-.536 4.752-2.563 4.752-4.939 0-2.404-1.281-3.872-4.459-4.994-2.269-.854-3.204-1.416-3.204-2.296 0-.748.562-1.496 2.298-1.496 1.924 0 3.15.614 3.844.909l.773-3.018c-.879-.427-2.082-.801-3.871-.882v-2.349h-2.615v2.536c-2.857.561-4.512 2.403-4.512 4.753 0 2.59 1.947 3.924 4.806 4.886 1.977.667 2.831 1.308 2.831 2.323 0 1.067-1.041 1.654-2.564 1.654-1.735 0-3.311-.561-4.432-1.174l-.801 3.124c1.014.587 2.75 1.068 4.538 1.147v2.535h2.616zM36.079 30.125a8.896 8.896 0 1 0 0 17.792 8.896 8.896 0 0 0 0-17.792zm3.491 9.326-5.027 5.027-.002-.003-.279.279-.004-.004-.004.004-.627-.627.004-.004-4.088-4.088 1.155-1.157 3.567 3.567 4.149-4.149-.004-.006 3.044-3.044 1.16 1.162-3.044 3.043z"></path><path d="M19.812 43.134H7.792V19.142h.016V4.867h19.134v7.175h7.175V27.101c.64-.104 1.292-.173 1.962-.173.974 0 1.915.127 2.821.345V9.479L29.504.083H3.025v47.834h16.788V48h8.185a12.081 12.081 0 0 1-3.28-4.866h-4.906z"></path></g>
			</svg>
			</span>
			<a href="#" class="sub-title">프리미엄</a>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href=/consult/items class="sub-inner my-title">상품목록</a>
			</li>
			<li>
				<i class="fa-solid fa-check"></i>
				<a href="/consult/payitems" class="sub-inner my-title">결제목록</a>
			</li>
		</ul>	
	</div>
	<div class="con-btn-box" style="align-items: center">
		<a class="btn-write btn con-btn" style="width: 100%; margin-top: 160px; " href="/member/consultantApply">상담사지원하기</a>	
	</div>
</div>
<script src="/resources/js/jquery-3.6.3.min.js"></script>

